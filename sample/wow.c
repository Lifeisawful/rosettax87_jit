...
typedef struct {
	...
	BenchFunc func;
	const char *expected;
	unsigned int weight; /* count from WoW.exe x87 profile (of ~75717 total) */
} Benchmark;
...
/*
 * Benchmarks weighted by WoW.exe x87 instruction frequency.
...
/* Register-popping arithmetic in WoW is 63% preceded by fmul mem,
...
void run_fist(clock_t *out_ticks, float *out_result) {
	/* In WoW, 79% of fistp is preceded by fsub (floor/truncate idiom) */
...
/* ================================================================
 *  Transcendental (rare in WoW but present)
 * ================================================================ */
...
int main() {
	/*
	 * Weight = raw instruction count from WoW.exe disassembly.
	...
	printf("| WEIGHTED   | %3lu%%  | %6s |  WoW-avg per 1M x87 ops |     |\n",
